...
| Mail driver | **Mailgun (EU region)**, `MAILGUN_ENDPOINT=api.eu.mailgun.net`, `MAIL_MAILER=mailgun` | D-15 + RESEARCH §Mail Driver Choice. Cheapest EU-compliant option for low volume contact form (~50/mo). DNS verify for `mg.dloazurpiscines.com` blocks first send — handled in Plan 06 cutover. |
