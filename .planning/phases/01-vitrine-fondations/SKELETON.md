...
| Object storage | **Cloudflare R2 (free 10 GB + zero egress)**, S3-compatible disk, `use_path_style_endpoint=true`, endpoint `https://r2.cloudflarestorage.com` | D-02 lock. **Wired in Phase 1, no uploads until Phase 2.** Buckets `dlo-azur-prod` + `dlo-azur-staging`, prefix convention `passages/{passage_uuid}/photos/`. |
