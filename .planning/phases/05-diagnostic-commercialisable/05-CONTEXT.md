...
- **V0 vs V0+V1:** the CDC splits work into V0 (MVP = SPEC scope + expert P0 corrections) and V1 (expert P1 corrections: treatment order, chlore-lock detection, eau-calcaire, stabilisant-bas, break-point, TH/chlore-total intake + lead qualification + optional photo). Several P1 items are correctness/safety-relevant (chlore-lock = faux diagnostic). **Planner needs to know: ship V0 only, or V0+V1?** Pending user confirmation.
...
- **User selection 2026-05-30:** leaning **"App complète (gros scope)"** — Phase 5 absorbs the mobile loop-closure (push re-test, offline carnet, début espace Pierre, possible native). NOT yet locked: user is seeking a third-party expert A-vs-B comparison (`05-EXPERT-COMPARISON-PROMPT.md`, conversion-focused) before final scope lock. This overrides the SPEC online-only boundary → SPEC will need re-spec if confirmed. North-star reminder threaded into all artifacts: **objective #1 = qualified leads / conversion for Pierre.**
- **Mobile loop-closure (push re-test, offline carnet, native wrapper):** the BLUEPRINT centers these but SPEC marks Phase 5 **online-only** and defers the carnet (DIAG-05). Recommendation: Phase 5 = the **minimal loop on web** (symptôme → "déjà tenté ?" → action-aware diagnosis → safe plan → 1-gesture rich escalation → lead → PDF); push/offline/carnet/espace-Pierre go to a dedicated **"mobile & rétention" phase** (matches BLUEPRINT §10.3-4 sequencing). The "déjà tenté ?" + action-aware diagnosis ARE recommended for Phase 5 (additive, web-compatible). **Confirm before planning.**
